...
def analyze_text(text):
    ...
    for i in text.lower():
        if i.isalpha():
            ...
            if len(current) >= 5 and current[0] == current[-1] and current not in word:
                words.append(current)
            current = ""
        return (len(unique_vowels)," ".join(words))
print(analyze_text("Amazing radar level stats 12345"))
#2
task=lambda s: " ".join(
...
print(top_k_words("hello hello word word word",2))
...
print(task("hello test abcde aabb 1234 world"))
#7
...
print(task("apple banana car 123test egg"))
#9
...
print(task("abc12 test123 word1 hello12345 a1b2c3"))
...
print(task("level hello radar world refer test"))
...
print(task("hello world apple banana sky"))
...
#5
task = lambda d: [k for k, v in sorted(d.items(), key=lambda x: (-x[1], x[0]))][:5]
print(task({'a': 5, 'b': 3, 'c': 5, 'd': 2, 'e': 4}))
...
#7
task = lambda a, b: set(x for x in a ^ b if x % 2 == 0)
print(task({1,2,3,4}, {3,4,5,6}))
...
print(task({'a': [1,2,3], 'b': [2,4], 'c': [5,7]}))
...
print(task({"hello", "world", "abcde", "aabb"}))
